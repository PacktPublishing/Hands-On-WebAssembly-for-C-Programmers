...
void drawDebugInformation(SDL_Renderer* renderer) {
    auto fpsSurface = TTF_RenderText_Blended(font, 
     (std::to_string(fps) + " Frames / Second").c_str(),
     {255, 255, 255, 255});
    ...
    auto averageTiming = 
        ...
        timings.size();
    auto timingSurface = TTF_RenderText_Blended(font,
     (std::to_string(averageTiming) + " ms render").c_str(),
     {255, 255, 255, 255});
    auto timingTexture = SDL_CreateTextureFromSurface(renderer,
                                                      timingSurface);
...
void drawScores(SDL_Renderer* renderer, int leftScore, int rightScore) {
    auto leftSurface = TTF_RenderText_Blended(font,
        std::to_string(leftScore).c_str(), {255, 0, 0, 255});
    auto leftTexture = SDL_CreateTextureFromSurface(renderer,
                                                    leftSurface);
    SDL_Rect leftScoreRect = {250, 20, 50, 50};
    SDL_RenderCopy(renderer, leftTexture, nullptr, &leftScoreRect);

    auto rightSurface = TTF_RenderText_Blended(font,
        std::to_string(rightScore).c_str(), {0, 0, 255, 255});
    auto rightTexture = SDL_CreateTextureFromSurface(renderer,
                                                     rightSurface);
